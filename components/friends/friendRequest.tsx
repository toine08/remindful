		...
		<View style={tw``}>
			...
			{requests.map((r) => (
				<View
					...
					style={tw`flex-row justify-between items-center m-1`}
				>
					...
					<View style={tw`flex-row`}>
						<Button
							label="Confirm"
							style={tw`mr-2 px-4 py-2 border-2 border-green-500 rounded bg-green-500`}
							textStyle={tw`text-dark-foreground dark:text-foreground`}
							onPress={() => handleRequest("accepted", r.friend_request_id)}
						/>
						<Button
							label="Delete"
							style={tw`px-4 py-2  border-2 border-red-400 rounded bg-red-400`}
							textStyle={tw`text-dark-foreground dark:text-foreground`}
