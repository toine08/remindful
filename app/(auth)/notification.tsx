...
function NotificationsList() {
    ...
    const { data } = supabase.storage.from("avatar").getPublicUrl(`${item.sender_id}/avatar.png`);
      ...
      <View style={tw`border-list`}>
        <TouchableWithoutFeedback onPress={() => console.log("Pressed!")}>
          <View style={tw`flex-row justify-between w-full `}>
            {avatarUrl && <Image source={{ uri: avatarUrl }} style={tw`h-14 w-14 rounded-full mr-2 bg-primary`} alt="Picture profile"/>}
            <Text style={tw`text-xl text-dark-foreground dark:text-foreground mb-2 font-bold w-5/6`}>
              {`${item.sender_username} thought about you ${
                Date.now() - new Date(item.sent_at).getTime() < 24 * 60 * 60 * 1000 ? "today" : "recently"
              }`}
            </Text>
          </View>
        </TouchableWithoutFeedback>
